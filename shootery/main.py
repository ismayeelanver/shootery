...
PATH = os.getcwd()
pygame.init()
pygame.mixer.Channel(0).play(pygame.mixer.Sound(PATH + "/song.mp3"), loops=-1)
...
async def main():
    ...
    # Calculate angle and direction vector
        ...
        if bullets and enemy.rect.collidepoint(bullets[-1]):
            pygame.mixer.Channel(1).play(pygame.mixer.Sound(PATH + "/kill.mp3"))
            enemy.health -= 20
            bullets.remove(bullet)
            score += 1
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                pygame.mixer.Channel(2).play(pygame.mixer.Sound(PATH + "/shoot.mp3"))
    
        if enemy.health <= 0:
            pygame.mixer.Channel(3).play(pygame.mixer.Sound(PATH + "/kill.mp3"))
            enemy = Enemy()
        ...
        if player_health <= 0:
            player_health = 100
            enemy = Enemy()
            pygame.mixer.Channel(4).play(pygame.mixer.Sound(PATH + "/die.mp3"))
